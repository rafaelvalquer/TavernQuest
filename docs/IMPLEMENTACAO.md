...
| 6. Contas Firebase | Telas de login/cadastro, repositório Firebase Auth e vínculo `Hero.userId` implementados. Regras Firestore/Storage, `firebase.json`, catálogo oficial de 50 missões e Function de validação/fan-out versionados; projeto real e `google-services.json` ainda dependem do console. |
...
| 12–13. Fotos e offline | Seleção de foto da galeria, cópia para armazenamento privado local, fila Room, WorkManager com rede disponível, transporte Firestore, upload Storage, correção do XP oficial e rollback de rejeição em transação Room implementados. Projeto Firebase e credenciais ainda são necessários para executar a sincronização online. |
