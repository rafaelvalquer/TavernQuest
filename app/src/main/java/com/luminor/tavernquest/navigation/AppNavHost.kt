...
@Composable fun AppNavHost(nav:NavHostController, modifier:Modifier=Modifier){ NavHost(nav,AppRoute.Splash,modifier){
 ...
 composable(AppRoute.TavernCreation){ TavernCreationScreen{ nav.navigateAndClear(AppRoute.Tavern) } }
 ...
 composable(AppRoute.QuestCompletion, listOf(navArgument("id"){type=NavType.StringType})){ QuestCompletionScreen(onDone={nav.navigateAndClear(AppRoute.Tavern)},onBack={nav.popBackStack()}) }
